...
def performFitByModel(x, y, model_key_a, model_key_b, method='leastsq', T=None, primFont=None, secFont=None):
    """
    Performs a fit on the provided data using two specified models summed together.
    Allows setting of custom primary and secondary fonts for dialog widgets.
    """
    ...
    # Create two lmfit Model instances, with unique prefixes
    ...
    # Combine the models by adding them. lmfit handles the rest.
    model = model1 + model2
    
    # Create parameters. lmfit automatically creates prefixed params.
    params = model.make_params()

    # Extract the combined parameter definitions for the dialog
    combined_params = {}
    init_dict = {}
    for p_name in params.keys():
        if p_name == "m1_T" or p_name == "m2_T": continue
        model_prefix, orig_name = p_name.split('_', 1)
        model_key = model_key_a if model_prefix == 'm1' else model_key_b
        combined_params[p_name] = models[model_key]['params'][orig_name]
        init_dict[p_name] = float(combined_params[p_name].get('init', 1.0))
    ...
    class BoundsDialog(simpledialog.Dialog):
        ...
        def body(self, master):
            toplevel = self.winfo_toplevel()
            toplevel.minsize(600, 60 + 30 * (len(combined_params) + 2))
            ...
            self.entries = {}
            for i, name in enumerate(combined_params, start=2):
                tk.Label(master, text=name, font=self.secFont).grid(row=i, column=0, padx=5, pady=2, sticky='w')
    ...
    y = np.asarray(y, float)
    if T is not None:
        # Add T as a parameter to both models, since it's a shared physical constant.
        # We make it a fixed value (vary=False).
        if 'm1_T' in params:
